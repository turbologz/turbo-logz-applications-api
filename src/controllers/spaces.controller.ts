...
export class SpacesController {
    ...
    async create(@requestBody() space: Space): Promise<Space> {
        const foundSpace = await this.spaceRepository.findOne({where: {name: space.name, orgId: space.orgId}});

        return foundSpace ? foundSpace : await this.spaceRepository.create(space);
    }
